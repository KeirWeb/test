...
const deepCopyStudents = students.map((i) => ({ ...i }));
...
console.log(
  students.map((i) => ({ ...i })).sort((a, b) => b.scores - a.scores)
);
console.log(students);
...
// const sortedByName = deepCopyStudents.sort((a, b) =>
//   a.name >= b.name ? 1 : -1
// );
const sortedByName = deepCopyStudents.sort((a, b) =>
  a.name >= b.name ? 1 : -1
...
let studentsWithMarriedNick = students.map((st) =>
  st.name === "Nick" ? { ...st, isMarried: true } : st
);
...
// const twoBestStudents = [
//   ...students.sort((a, b) => b.scores - a.scores).slice(0, 2),
// ];
// console.log(twoBestStudents);

// let best1 = students[0];
// let best2 = students[1];
// for (let i = 0; i <= students.length; i++) {
//   if (best1.scores < students[i].scores) {
//     best1 = students[i];
//   } else if (best2.scores < students[i].scores) {
//     best2 = students[i];
//   }
// }
// console.log(students);
...
function addFriends(students) {
  ...
  function getFriendsList(student) {
    return studentsNames.filter((name) => name !== student.name);
  }
  const studentsWithFriends = students.map((st) => ({
    ...st,
    friends: getFriendsList(st),
  }));
